...
const registrarComidas = async (req, res) => {
    const { paciente, tipoComida } = req.body;

    if (!mongoose.Types.ObjectId.isValid(paciente)) {
        return res.status(404).json({ msg: `Lo sentimos, debe ser un id válido` });
    }

    // Obtener la fecha actual sin la hora (para comparar solo por día)
    const hoy = new Date();
    ...
    // Buscar si ya hay una comida del mismo tipo para el paciente hoy
    const comidaExistente = await ComidasPaciente.findOne({
        paciente,
        tipoComida,
    ...
    if (comidaExistente) {
        return res.status(400).json({ msg: `Ya se ha registrado un(a) ${tipoComida} para este paciente hoy.` });
    }

    const nuevaComida = await ComidasPaciente.create(req.body);
    res.status(200).json({
        msg: `Registro de comida exitosamente ${nuevaComida._id}`,
        nuevaComida
    });
};
